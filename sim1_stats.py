...
class SimulationCLI:
    ...
    def list_results(self):
        ...
        if not results:
            print("No results found in results/sim1/ directory")
            return
    ...
    def plot(self, *result_files: str, pattern: str = None):
        ...
        Args:
            result_files: Paths to JSON result files (relative to results/sim1/ or absolute)
            pattern: Glob pattern to match files (e.g., "compare_*.json")

        Examples:
            # Plot specific files
            python run_stats.py plot results/sim1/file1.json results/sim1/file2.json
